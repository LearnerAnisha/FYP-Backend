...
def generate_esewa_signature(
    total_amount: str, transaction_uuid: str, product_code: str
) -> str:
    """
...
def verify_esewa_signature(data_string: str, received_signature: str) -> bool:
    ...
    raw_sig = hmac.new(secret, data_string.encode("utf-8"), hashlib.sha256).digest()
...
def verify_payment_with_esewa(transaction_uuid: str, total_amount: str) -> dict:
    ...
    try:
        ...
        data = response.json()
        logger.info(
            "eSewa verification response | txn=%s | status=%s",
            transaction_uuid,
            data.get("status"),
        )
        return data
        ...
        raise requests.RequestException(
            "eSewa verification API timed out. Please try again."
        )

    except requests.ConnectionError:
        logger.error("Cannot connect to eSewa API | txn=%s", transaction_uuid)
        raise requests.RequestException(
            "Unable to connect to eSewa servers. Check your internet."
        )

    except requests.HTTPError as e:
        logger.error(
            "eSewa API HTTP error %s | txn=%s", response.status_code, transaction_uuid
        )
        raise requests.RequestException(
            f"eSewa API returned error {response.status_code}."
        )
