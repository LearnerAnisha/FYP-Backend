...
class CropSuggestionListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = CropSuggestion.objects.all().select_related(
        ...
        if search:
            queryset = queryset.filter(
                Q(crop_name__icontains=search) | Q(growth_stage__icontains=search)
            )
...
class ScanResultListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = queryset.filter(
                Q(crop_type__icontains=search) | Q(disease__icontains=search)
            )
...
class AdminSubscriptionListView(generics.ListAPIView):
    ...
    List all subscriptions with search and filtering.
    """
    ...
    def get_queryset(self):
        qs = Subscription.objects.select_related("user", "payment").all()
...
class AdminSubscriptionDetailView(generics.RetrieveUpdateDestroyAPIView):
    """
    GET/PATCH/DELETE /admin/subscriptions/<pk>/
    """

    queryset = Subscription.objects.select_related("user", "payment").all()
    serializer_class = AdminSubscriptionSerializer
    permission_classes = [IsAuthenticated, IsAdminUser]
    ...
    def perform_update(self, serializer):
        ...
        log_admin_action(
            admin_user=self.request.user,
            action="update",
            description=f"Updated subscription for {sub.user.email}",
    ...
    def perform_destroy(self, instance):
        log_admin_action(
            admin_user=self.request.user,
            action="delete",
            description=f"Deleted subscription for {instance.user.email}",
            target_user=instance.user,
            request=self.request,
        )
        instance.delete()
